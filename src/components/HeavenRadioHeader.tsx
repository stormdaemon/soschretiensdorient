...
export const HeavenRadioHeader: React.FC = () => {
  ...
  useEffect(() => {
    const updateHeight = () => {
      ...
      document.documentElement.style.setProperty('--heaven-header-height', `${height}px`);
    };
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto flex flex-col gap-2 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4 sm:px-6 lg:px-8">
        <div className="flex flex-wrap items-center gap-3 text-red-100">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[0.6rem] uppercase tracking-[0.35em] text-red-200/80 sm:text-xs">Live</span>
          </div>
          <div className="flex items-center gap-2">
            <MusicalNoteIcon className="h-4 w-4 text-red-100 sm:h-5 sm:w-5" aria-hidden="true" />
            <p className="text-base font-semibold text-white md:text-lg">Heaven Radio • Partenaire</p>
          </div>
          <p className="text-xs leading-tight text-red-100/80 md:text-sm">100% Louange et Adoration</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 sm:gap-4">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="flex items-center gap-2 text-red-100">
              ...
              <input
                ...
                className="h-1 w-28 cursor-pointer appearance-none rounded-full bg-red-200/40 accent-red-100 sm:w-32"
          ...
          <a
            ...
            className="hidden text-sm font-medium text-red-100 transition hover:text-white md:inline"
          >
            Ouvrir dans une nouvelle fenêtre
          </a>

          {error && (
            <p className="w-full text-xs text-red-100/90 sm:w-auto sm:text-sm" role="status">
              {error}
            </p>
          )}
        </div>
